...
namespace Unity.XR.XREAL
{
    ...
    public class XREALAudioCapture : IDisposable
    {
        /// <summary>
        ...
        public static XREALAudioCapture Create()
        {
            XREALAudioCapture capture = new XREALAudioCapture();
            return capture;
        }

        /// <summary> Default constructor. </summary>
        public XREALAudioCapture()
        {
            IsRecording = false;
        }
        ...
        {
            Debug.Log($"[AudioCapture] StartRecordingAsync: IsRecording={IsRecording}, volFactorMic={volumeFactorMic}, volFactorApp={volumeFactorApp}");
            ...
            else
            {
                ...
                {
                    Debug.LogWarning($"[AudioCapture] StartRecordingAsync: {ex.Message}\n{ex.StackTrace}");
                    result.resultType = CaptureResultType.UnknownError;
                    onStartedRecordingAudioCallback?.Invoke(result);
                    throw;
                }
                ...
                XREALAndroidPermissionsManager.GetInstance().RequestAndroidPermission("android.permission.RECORD_AUDIO").ThenAction((requestResult) =>
                {
                    if (requestResult.IsAllGranted)
                    {
                        XREALAndroidPermissionsManager.GetInstance().RequestScreenCapture().ThenAction((AndroidJavaObject mediaProjection) =>
                        {
                            ...
                            {
                                Debug.LogError("[AudioCapture] Screen capture is denied by user.");
                                var result = new AudioCaptureResult();
                                result.resultType = CaptureResultType.UnknownError;
                    ...
                    {
                        Debug.LogError("[AudioCapture] Record audio need the permission of 'android.permission.RECORD_AUDIO'.");
                        var result = new AudioCaptureResult();
                        result.resultType = CaptureResultType.UnknownError;
                        onAudioModeStartedCallback?.Invoke(result);
        ...
        private void StartAudioMode(CameraParameters setupParams, OnAudioModeStartedCallback onAudioModeStartedCallback)
        {
            ...
            if (setupParams.frameRate <= 0)
                Debug.LogWarning("[AudioCapture] frameRate need to be bigger than zero");
