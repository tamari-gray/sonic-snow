...
namespace Unity.XR.XREAL
{
    ...
    public class XREALVideoCapture : IDisposable
    {
        /// <summary> Default constructor. </summary>
        public XREALVideoCapture(FrameCaptureContext context)
        {
            IsRecording = false;
        ...
        internal FrameCaptureContext m_CaptureContext;
        ...
        public void StartRecordingAsync(string filename, OnStartedRecordingVideoCallback onStartedRecordingVideoCallback)
        {
            ...
            if (XREALPlugin.GetDeviceType() == XREALDeviceType.XREAL_DEVICE_TYPE_LIGHT)
            {
        ...
        {
            Debug.LogFormat("[VideoCapture] StartRecordingAsync: IsRecording={0}, volFactorMic={1}, volFactorApp={2}", IsRecording, volumeFactorMic, volumeFactorApp);
            ...
            else
            {
                try
                {
                    ...
                    ((XREALRecordBehaviour)behaviour).SetOutPutPath(filename);
                ...
                {
                    Debug.LogWarningFormat("[VideoCapture] StartRecordingAsync: {0}\n{1}", ex.Message, ex.StackTrace);
                    result.resultType = CaptureResultType.UnknownError;
                    onStartedRecordingVideoCallback?.Invoke(result);
                    throw;
                }
        ...
        public void StartVideoModeAsync(CameraParameters setupParams, OnVideoModeStartedCallback onVideoModeStartedCallback, bool autoAdaptBlendMode = true)
        {
            if (autoAdaptBlendMode)
            {
                ...
                {
                    Debug.LogWarningFormat("[VideoCapture] AutoAdaptBlendMode : {0} => {1}", setupParams.blendMode, blendMode);
            ...
            Debug.LogFormat("[VideoCapture] StartVideoModeAsync: audioState={0}, blendMode={1}", setupParams.audioState, setupParams.blendMode);
            bool recordMic = setupParams.CaptureAudioMic;
            bool recordApp = setupParams.CaptureAudioApplication;
            if (recordApp)
            {
                XREALAndroidPermissionsManager.GetInstance().RequestAndroidPermission("android.permission.RECORD_AUDIO").ThenAction((requestResult) =>
                {
                    if (requestResult.IsAllGranted)
                    {
                        XREALAndroidPermissionsManager.GetInstance().RequestScreenCapture().ThenAction((AndroidJavaObject mediaProjection) =>
                        {
                            ...
                            {
                                Debug.LogError("[VideoCapture] Screen capture is denied by user.");
                                var result = new VideoCaptureResult();
                                result.resultType = CaptureResultType.UnknownError;
                                onVideoModeStartedCallback?.Invoke(result);
                                XREALCallbackHandler.InvokeXREALError(XREALErrorCode.PermissionDenyError, NativeConstants.ScreenCaptureDenyErrorTip);
                            }
                        });
                    }
                    else
                    {
                        Debug.LogError("[VideoCapture] Record audio need the permission of 'android.permission.RECORD_AUDIO'.");
                        var result = new VideoCaptureResult();
                        result.resultType = CaptureResultType.UnknownError;
                        onVideoModeStartedCallback?.Invoke(result);
                        XREALCallbackHandler.InvokeXREALError(XREALErrorCode.PermissionDenyError, NativeConstants.PermissionDenyErrorTip);
                    }
                });
            }
            else if (recordMic)
        ...
        private void StartVideoMode(CameraParameters setupParams, OnVideoModeStartedCallback onVideoModeStartedCallback)
        {
            ...
            if (setupParams.frameRate <= 0)
                Debug.LogWarningFormat("[PhotoCapture] frameRate need to be bigger than zero");
        ...
        public void StartVideoModeAsync(CameraParameters setupParams, AudioState audioState, OnVideoModeStartedCallback onVideoModeStartedCallback, bool autoAdaptBlendMode = true)
        {
        ...
        public delegate void OnStartedRecordingVideoCallback(XREALVideoCapture.VideoCaptureResult result);
        ...
        public delegate void OnVideoCaptureResourceCreatedCallback(XREALVideoCapture captureObject);
        ...
        public delegate void OnVideoModeStartedCallback(XREALVideoCapture.VideoCaptureResult result);
        ...
        public delegate void OnVideoModeStoppedCallback(XREALVideoCapture.VideoCaptureResult result);
        ...
        ///                       file system.</param>
        public delegate void OnStoppedRecordingVideoCallback(XREALVideoCapture.VideoCaptureResult result);
    }

    public class XREALVideoCaptureUtility
    {
        ...
        public static void CreateAsync(bool showHolograms, XREALVideoCapture.OnVideoCaptureResourceCreatedCallback onCreatedCallback)
        {
            XREALVideoCapture capture = new XREALVideoCapture(FrameCaptureContextFactory.Create());
