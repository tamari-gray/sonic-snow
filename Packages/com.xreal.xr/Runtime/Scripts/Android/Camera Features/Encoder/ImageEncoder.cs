...
namespace Unity.XR.XREAL
{
    ...
    public class ImageEncoder : IEncoder
    {
        ...
        public void Start()
        {
            XREALMainThreadDispatcher.OnUpdate -= Update;
            XREALMainThreadDispatcher.OnUpdate += Update;
        }
                ...
                else if (req.done)
                {
                    ...
                    if (m_EncodeTempTex == null)
                    {
                        m_EncodeTempTex = new Texture2D(
                            ...
                            m_CameraParameters.captureSide == CaptureSide.Both ? (m_CameraParameters.cameraResolutionHeight / 2) : m_CameraParameters.cameraResolutionHeight,
                    ...
                    if (task.OnReceive != null)
                    {
                        if (m_EncodeTempTex.width != task.Width || m_EncodeTempTex.height != task.Height)
                        {
                            ...
                            Debug.Log($"[BlendCamera] need to scale the texture which origin width:{m_EncodeTempTex.width} and out put width:{task.Width}");
